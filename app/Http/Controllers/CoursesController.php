...
class CoursesController extends Controller
{
    ...
    public function index(Request $request, $cat = null)
    {
        ...
        if (isset($cat)) {
            ...
            $all_cources = Course::paginate(10);
        }

        $courses = Course::orderBy('created_at', 'desc')->get();


        $courses_popular = DB::table('courses')
            ->join('applicants', 'applicants.course_id', '=', 'courses.id')
            ->selectRaw('courses.id, courses.title, courses.slug, courses.description, courses.image, courses.created_at, courses.updated_at, courses.deleted_at, SUM(applicants.price) AS price')
            ->groupBy('courses.id', 'courses.title', 'courses.slug', 'courses.description', 'courses.image', 'courses.created_at', 'courses.updated_at', 'courses.deleted_at')
            ->orderBy('price', 'DESC')
            ->take(8)
            ->get();
        return view('cources.cources', compact('all_cources', 'cats', 'courses_popular', 'courses'));
    }
    ...
    public function filterCourse(Request  $request)
    {
        ...
        $c = Course::orderBy('created_at', 'desc')->get();
        if ($request->search) {
            $all_cources->whereTranslation('title', 'like', '%' . $request->search . '%')->paginate(10);
        }
        if ($request->category_id) {
            $all_cources->whereIn('category_id', $request->category_id)->paginate(10);
        }
        if ($request->min_price) {
            $all_cources->where('price', '>=', $request->min_price)->paginate(10);
        }
        if ($request->max_price) {
            $all_cources->where('price', '<=', $request->max_price)->paginate(10);
        }
        $all_cources = $all_cources->paginate(10);
